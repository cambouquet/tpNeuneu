		...
		if(distF == 0) {
			...
			for (ObjetDessinable obj : localListeObjet) {
				...
				if (dist == 0) {
					...
					//On vérifie que l'objet sur la même case est bien du type Nourriture
		...
		if(distN == 0) {
			...
			for (ObjetDessinable obj : localListeObjet) {
				...
				if (dist == 0) {
					...
					//On vérifie que l'objet sur la même case est bien du type Neuneu
					if (obj instanceof Neuneu) {
						Neuneu neuneu = (Neuneu) obj;
						//Création d'un neuneu
						
						//Vérification des niveaux d'énergie et destruction(s) éventuelle(s)
